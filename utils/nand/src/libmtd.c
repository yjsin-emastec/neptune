...
static inline int mtd_ioctl_error(const struct mtd_dev_info *mtd, int eb, const char *sreq)
{
...
static int mtd_xlock(const struct mtd_dev_info *mtd, int fd, int eb, int req, const char *sreq)
{
    int ret;
...
int mtd_read(const struct mtd_dev_info *mtd, int fd, int eb, int offs, void *buf, int len)
{
    int ret, rd = 0;
    off_t seek;
...
int mtd_write(const struct mtd_dev_info *mtd, int fd, int eb, int offs, void *buf, int len)
{
    int ret;
    off_t seek;
...
int do_oob_op(libmtd_t desc, const struct mtd_dev_info *mtd, int fd, uint64_t start, uint64_t length, void *data, unsigned int cmd64, unsigned int cmd)
{
    int ret;
...
int mtd_read_oob(libmtd_t desc, const struct mtd_dev_info *mtd, int fd, uint64_t start, uint64_t length, void *data)
{
    return do_oob_op(desc, mtd, fd, start, length, data, MEMREADOOB64, MEMREADOOB);
}

int mtd_write_oob(libmtd_t desc, const struct mtd_dev_info *mtd, int fd, uint64_t start, uint64_t length, void *data)
{
    return do_oob_op(desc, mtd, fd, start, length, data, MEMWRITEOOB64, MEMWRITEOOB);
}

int mtd_write_img(const struct mtd_dev_info *mtd, int fd, int eb, int offs, const char *img_name)
{
    ...
    if (offs < 0 || offs >= mtd->eb_size) {
        errmsg("bad offset %d, mtd%d eraseblock size is %d", offs, mtd->mtd_num, mtd->eb_size);
        errno = EINVAL;
        return -1;
    }
    if (offs % mtd->subpage_size) {
        errmsg("write offset %d is not aligned to mtd%d min. I/O size %d", offs, mtd->mtd_num, mtd->subpage_size);
        errno = EINVAL;
        return -1;
    }
    ...
    len = st.st_size;
    if (len % mtd->subpage_size) {
        errmsg("size of \"%s\" is %d byte, which is not aligned to " "mtd%d min. I/O size %d", img_name, len, mtd->mtd_num, mtd->subpage_size);
        errno = EINVAL;
        goto out_close;
    }
